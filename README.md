...
- **데이터 주도 스킬**: 캐릭터 스탯(`src/data/characters.json`)과 스킬(`src/data/skills.json`)은
  전부 데이터 파일. 밸런스 조정은 이 두 파일만 고치면 된다. 스킬 `kind`는 다섯 가지 —
  `damage`(직접 피해, `hits`로 다단히트·`ignoreDef`로 방어 무시 가능) / `heal`(자기 회복) /
  `buff`(자기 자신에게 상태 효과만) / `drain`(소량 피해 + 상대 MP 직접 감소) /
  `restore_mp`(자기 MP 회복, 침묵이면 막힘). `stat_mod` 효과는 `atk`(주는 피해)와
  `def`(받는 피해) 두 스탯을 지원하고, 서로 다른 효과끼리는 배율이 곱해져 함께 적용된다.
...
각 캐릭터는 기본 공격 1개 + 고유 스킬 4개(공통 회복 포함) = 총 5개 스킬을 갖는다.

| | 기본 공격 | 스킬 1 | 스킬 2 | 스킬 3 |
|---|---|---|---|---|
| **전사** | 베기 | 강타 — 고배율 타격 + 상대 공격력 감소 | 방어 태세 — 받는 피해 절반 (버프) | 투지 — 자신 공격력 40% 증가 (버프) |
| **궁수** | 사격 | 관통 사격 — 타격 + 출혈(dot) | 속사 — 2연타 | 조준 사격 — 방어 무시 정밀 타격 |
| **마법사** | 마력탄 | 파열 마법 — 최고배율 타격 + 침묵 | 마나 번 — 소량 피해 + 상대 MP 직접 감소 | 집중 — 자기 MP 회복(침묵 시 막힘) |

회복은 세 캐릭터가 공유한다. 전사는 "버프로 판을 짜는" 지속 압박형, 궁수는 "다양한 사격 기술"로
상황에 맞춰 고르는 딜러, 마법사는 "내 자원은 지키고 상대 자원은 조이는" 자원 관리형이라는
컨셉을 스킬 구성으로 나눠서 표현했다.
